...
func TestRollbackToHeight(t *testing.T) {
	...
	accSeq := app.Account.GetSequence()
	...
	for i := 2; i < height; i++ {
		if key, value := build(t, app, int64(i), accSeq); i > newHeight {
	...
	for i := newHeight + 1; i < height; i++ {
		key, value := build(t, app, int64(i), accSeq)
		newState[key] = value
		accSeq += 1
	}
	{
...
func build(t *testing.T, app testapp.TestApp, height int64, seq uint64) (string, string) {
	...
	_, err := app.FinalizeBlock(context.Background(), &abcitypes.RequestFinalizeBlock{
		Txs:    [][]byte{app.ToTx(t, key, value, seq)},
		Height: height,
	})
	require.NoError(t, err)
