...
func TestBuild(t *testing.T) {
	...
	for description, test := range tests {
		t.Run(description, func(t *testing.T) {
			app := testapp.NewTest(t, testapp.ChainID, true)
			...
			inclusionListTxs := app.ToTxs(t, test.inclusionList, app.Account.GetSequence())
			mempoolTxs := app.ToTxs(t, test.mempool, uint64(len(inclusionListTxs)))
			...
			// Block store.
			genesisHeader, err := app.BlockStore.BlockByHeight(uint64(preBuildInfo.GetLastBlockHeight()))
			require.NoError(t, err)
			gotBlock, err := app.BlockStore.HeadBlock()
			require.NoError(t, err)
			...
			// Eth state db.
			ethState, err := state.New(ethStateRoot, app.EthStateDB, nil)
			require.NoError(t, err)
...
func TestRollback(t *testing.T) {
	...
	genesisHeader, err := app.BlockStore.HeadHeader()
	require.NoError(t, err)
	...
	block, err := b.Build(context.Background(), &builder.Payload{
		Timestamp:            app.Genesis.Time + 1,
		InjectedTransactions: bfttypes.ToTxs(app.ToTxs(t, kvs, app.Account.GetSequence())),
	})
	require.NoError(t, err)
	require.NotNil(t, block)
	require.NoError(t, app.BlockStore.UpdateLabels(block.Header.Hash, block.Header.Hash, block.Header.Hash))

	// Eth state db before rollback.
	ethState, err := state.New(block.Header.StateRoot, app.EthStateDB, nil)
	require.NoError(t, err)
	...
	// Block store.
	height, err := app.BlockStore.Height()
	require.NoError(t, err)
	...
	ethState, err = state.New(genesisHeader.StateRoot, app.EthStateDB, nil)
	require.NoError(t, err)
	require.Equal(t, ethState.GetStorageRoot(contracts.L2ApplicationStateRootProviderAddr), gethtypes.EmptyRootHash)

	// Tx store.
	for _, tx := range bfttypes.ToTxs(app.ToTxs(t, kvs, app.Account.GetSequence())) {
